...
static void checkWavesPerEU(Module &M, TargetMachine &TM) {
  for (Function &F : M) {
    ...
    // Widen the range if the attribute is wider.
    // TODO: That means the attribute is worse. Do we have to do it?
    if (WavesPerEUAttr.has_value()) {
      unsigned MinFromAttr = WavesPerEUAttr->first;
      unsigned MaxFromAttr = WavesPerEUAttr->second.has_value()
                                 ? *(WavesPerEUAttr->second)
                                 : MaxWavesPerEU;
      Min = std::min(Min, MinFromAttr);
      Max = std::max(Min, MaxFromAttr);
    }

    // Clamp the range to the max.
